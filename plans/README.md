# 実装計画インデックス(v1.4.0 リリース時点)

最終更新: 2026-08-07 / 残り計画: **1 件**

計測ギャップ解消の当初計画は全 11 件だった。**01〜09 と 11 は v1.2.0 で
実装・出荷済み**であり、後から追加した外部pprofコードレベル解析も実装を完了した。
完了した項目の計画文書は削除済みであり、残るのは
**[10 複数台横断計測](./10-multi-host.md)** だけである。
...
削除直前の固定commit `357d94f51fed7eb08aba777ab1ee6756b7a96090`を参照すること。
例: `git show 357d94f:plans/07-runtime-profiles.md`。`HEAD^`や「本commitの1つ前」のような
時間とともに壊れる案内は使わない。実装の**正**は計画文書ではなく
...
**規模: raw 26.5 日 / +30% buffer で 34.5 日(≈5〜6 週間)。対象リリース v1.5.0 以降。**
...
この59.0日/77日は、当初の全11計画だけを対象とした見積もりである。
